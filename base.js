...
'cms':{//系统
	...
	'pre':function(){//代码高亮
		...
		if($("pre code").length){
			...
			$.getScript("//s3.pstatp.com/cdn/expire-1-M/highlight.js/9.12.0/highlight.min.js", function(){
	...
	'flickity':function(){//手机滑动
		if($(".ff-gallery").length){
			...
			$("<link>").attr({
				rel: "stylesheet",
				type: "text/css",
				href: "//s0.pstatp.com/cdn/expire-1-M/flickity/2.1.1/flickity.min.css"
			}).appendTo("head");
			$.getScript("//s0.pstatp.com/cdn/expire-1-M/flickity/2.1.1/flickity.pkgd.min.js", function(){
...
'search': {//搜索
	...
	'autocomplete': function(){//3.5增加搜索结果控制 4.1update
		...
		if( $limit > 0){
			...
			$.getScript("//s2.pstatp.com/cdn/expire-1-M/jquery.devbridge-autocomplete/1.4.7/jquery.autocomplete.min.js", function(response, status) {
				$('.ff-wd').autocomplete({
...
'score': {//评分
	'raty': function(){
		if( $('.ff-score').length ){
			...
			$("<link>").attr({ rel: "stylesheet",type: "text/css",href: "//s3.pstatp.com/cdn/expire-1-M/raty/2.7.1/jquery.raty.min.css"}).appendTo("head");
			//
			$.getScript("//s1.pstatp.com/cdn/expire-1-M/raty/2.7.1/jquery.raty.min.js", function(response, status) {
